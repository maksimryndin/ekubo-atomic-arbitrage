...
fn multihop_swap(
    ...
    router_address: Felt,
    token_address: Felt,
    multihop_swap_selector: Felt,
) -> Call {
    let specified_amount = split.specified_amount;
    let calldata: Vec<Felt> = iter::once(Felt::from(split.route.len()))
        .chain(split.route.into_iter().flat_map(|node| {
            let RouteNode {
                pool_key:
                    PoolKey {
                        token0,
        ...
        opportunities
            ...
            .find_map(|opportunity| {
                ...
                let call = if splits.len() == 1 {
                    ...
                    multihop_swap(split, router_address, token_address, multihop_swap_selector)
                } else {
                    todo!()
                };
